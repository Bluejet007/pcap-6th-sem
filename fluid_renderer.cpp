    ...
    */
    ...
    static void initGL()
    {
        ...
        for (int i = 0; i < 2; ++i) {
            auto err1 = cudaGraphicsGLRegisterBuffer(&g_cudaPos[i], g_vbo[i],
                cudaGraphicsMapFlagsWriteDiscard);

            auto err2 = cudaGraphicsGLRegisterBuffer(&g_cudaVel[i], g_velVBO[i],
                cudaGraphicsMapFlagsWriteDiscard);

            if (err1 != cudaSuccess || err2 != cudaSuccess) {
                printf("CUDA Registration failed at index %d!\n", i);
            }
        }
    ...
    extern "C" void cudaStepSimulation(
        float time,
        cudaGraphicsResource* pos[2],
        cudaGraphicsResource* vel[2],
        int& pingPong,
        uint N,
